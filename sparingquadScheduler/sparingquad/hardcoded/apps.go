// Package hardcoded contains an implementation of sparingquad.InterferenceModel,
...
// HardcodedSlowDowns is an implementation of sparingquad.InterferenceModel, where the
// slowdowns among all applications are known and hardcoded.
type HardcodedSlowDowns struct {
	sparingquadLabelKey string
}

// New returns a new HardcodedSlowDowns with the given label key (the one that
// is used by SparingQuadPlugin to track its applications).
func New(sparingquadLabelKey string) *HardcodedSlowDowns {
	return &HardcodedSlowDowns{
		sparingquadLabelKey: sparingquadLabelKey,
	}
}

// Attack implements sparingquad.InterferenceModel; see the documentation there for
// more information.
func (m *HardcodedSlowDowns) Attack(attacker, occupant *corev1.Pod) (float64, error) {
	occPodCategory, _ := parseAppCategory(occupant.Labels[m.sparingquadLabelKey])
	//  occupant   ^^^   Pod's label's value must have been
	// validated back when it was first scheduled on the Node
	newPodCategory, err := parseAppCategory(attacker.Labels[m.sparingquadLabelKey])
	if err != nil {
		return -1, err
	}
...
// ToInt64Multiplier implements sparingquad.InterferenceModel; see the documentation
...
// the 4 appCategories that are to be examined by the SparingQuadPlugin are as
// follows:
...
// slowDownMatrix is a type alias for internal use in SparingQuadPlugin.
...
var slowDowns = slowDownMatrix{
	...
	catC: map[appCategory]float64{
		catA: 1.21,
		catB: 3.09,
		catC: 1.16, // slowdown of catC when attacked by catC = 1.26
		catD: 1.18,
	},
