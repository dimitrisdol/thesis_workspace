// Package sparingquad contains an out-of-tree plugin based on the Kubernetes
...
import (
	...
	"github.com/dimitrisdol/sparingquadScheduler/sparingquad/hardcoded"
...
const (
	...
	// sparingquadLabelKey is the key of the Kubernetes Label which every
	// application that needs to be tracked by SparingQuadPlugin should have.
	sparingquadLabelKey = "category"
)

// SparingQuadPlugin is an out-of-tree plugin for the kube-scheduler, which takes into
...
type SparingQuadPlugin struct {
	handle framework.Handle
	model  InterferenceModel
}

var (
	_ framework.Plugin          = &SparingQuadPlugin{}
	_ framework.FilterPlugin    = &SparingQuadPlugin{}
	_ framework.ScorePlugin     = &SparingQuadPlugin{}
	_ framework.ScoreExtensions = &SparingQuadPlugin{}
)

// New instantiates a SparingQuadPlugin.
func New(configuration runtime.Object, f framework.Handle) (framework.Plugin, error) {
	return &SparingQuadPlugin{
		handle: f,
		model:  hardcoded.New(sparingquadLabelKey),
	}, nil
}

// Name returns the official name of the SparingQuadPlugin.
func (_ *SparingQuadPlugin) Name() string {
	return Name
}

// findCurrentOccupants returns all Pods that are being tracked by SparingQuadPlugin
...
func (_ *SparingQuadPlugin) findCurrentOccupants(nodeInfo *framework.NodeInfo) []*corev1.Pod {
	ret := make([]*corev1.Pod, 0, 4)
	for _, podInfo := range nodeInfo.Pods {
		for key := range podInfo.Pod.Labels {
			if sparingquadLabelKey == key {
...
func (ap *SparingQuadPlugin) Filter(
	ctx context.Context,
	...
	// If the given Pod does not have the sparingquadLabelKey, approve it and let
	// the other plugins decide for its fate.
	if _, exists := pod.Labels[sparingquadLabelKey]; !exists {
		klog.V(2).Infof("blindly approving Pod '%s/%s' as it does not have SparingQuadPlugin's label %q", pod.Namespace, pod.Name, sparingquadLabelKey)
		return framework.NewStatus(framework.Success, "pod is not tracked by SparingQuadPlugin")
	}

	// For the Node at hand, find all occupant Pods tracked by SparingQuadPlugin.
	...
	// If the Node is full (i.e., 4 applications tracked by SparingQuadPlugin are
	// already scheduled on it), filter it out.
	case 4:
		klog.V(2).Infof("filtering Node %q out because 2 SparingQuadPlugin applications are already scheduled there", nodeName)
		return framework.NewStatus(framework.Unschedulable, fmt.Sprintf("Node '%s' already has 2 SparingQuadPlugin occupants", nodeName))
	...
	case 3:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[sparingquadLabelKey],)
			klog.V(2).Infof(msg)
	...
	case 1:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[sparingquadLabelKey], occ.Namespace, occ.Name, occ.Labels[sparingquadLabelKey])
			klog.V(2).Infof(msg)
			return framework.NewStatus(framework.Unschedulable, msg)
		}
		fallthrough
...
//
// In the case of SparingQuadPlugin, scoring is reversed; i.e., higher score indicates
// worse scheduling decision.
// This is taken into account and "fixed" later, during the normalization.
func (ap *SparingQuadPlugin) Score(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	nodeName string,
	...
	// If the given Pod does not have the sparingquadLabelKey, approve it and let
	// the other plugins decide for its fate.
	if _, exists := p.Labels[sparingquadLabelKey]; !exists {
		klog.V(2).Infof("blindly scoring Pod '%s/%s' as it does not have SparingQuadPlugin's label %q", p.Namespace, p.Name, sparingquadLabelKey)
...
// ScoreExtensions returns the SparingQuadPlugin itself, since it implements the
// framework.ScoreExtensions interface.
func (ap *SparingQuadPlugin) ScoreExtensions() framework.ScoreExtensions {
	return ap
}
...
//
// In the case of the SparingQuadPlugin, its "Score" method produces scores of reverse
...
func (_ *SparingQuadPlugin) NormalizeScore(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	...
	// When no Pod (tracked by SparingQuadPlugin) is scheduled on the Node,
	// maxScore will be 0.
	if maxScore == 0 {
		for i := range scores {
